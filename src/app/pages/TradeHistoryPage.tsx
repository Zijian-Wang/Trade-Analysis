...
export function TradeHistoryPage({
  ...
  const handleCopy = async () => {
    await navigator.clipboard.writeText(
      JSON.stringify(filteredAndSortedTrades, null, 2),
    )
    setCopied(true)
  ...
  const handleDownload = () => {
    const dataStr =
      ...
      encodeURIComponent(JSON.stringify(filteredAndSortedTrades, null, 2))
    const downloadAnchorNode = document.createElement('a')
    downloadAnchorNode.setAttribute('href', dataStr)
    downloadAnchorNode.setAttribute(
      'download',
      `trade_history_${new Date().toISOString().split('T')[0]}.json`,
    )
  ...
  const hasActiveFilters =
    searchQuery || directionFilter !== 'all' || setupFilter !== 'all'
  ...
  return (
    <div className="min-h-screen transition-colors duration-300 dark:bg-gradient-to-br dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 bg-gradient-to-br from-gray-50 via-white to-gray-100 pt-20">
      <main className="max-w-[1400px] mx-auto px-4 sm:px-6 py-6">
        ...
        <Card
          className={`p-6 shadow-sm transition-all duration-300 ${
            isDarkMode
        ...
        >
          ...
          <div className="mb-6 space-y-4">
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              >
                <Select.Trigger
                  className={`min-w-[140px] px-3 py-2 border rounded-lg text-sm transition-colors flex items-center justify-between ${
                    isDarkMode
                ...
                <Select.Portal>
                  <Select.Content
                    position="popper"
                    sideOffset={4}
                    className={`w-[var(--radix-select-trigger-width)] overflow-hidden rounded-lg shadow-lg ${
                      isDarkMode
                    ...
                    <Select.Viewport className="p-1">
                      <Select.Item
                        ...
                        className={`px-3 py-2 text-sm rounded cursor-pointer outline-none ${
                          isDarkMode
                            ? 'text-white hover:bg-gray-600 data-[highlighted]:bg-gray-600'
              ...
              <Select.Root value={setupFilter} onValueChange={setSetupFilter}>
                <Select.Trigger
                  className={`min-w-[160px] px-3 py-2 border rounded-lg text-sm transition-colors flex items-center justify-between ${
                    isDarkMode
                      ? 'bg-gray-700 border-gray-600 text-white'
                      : 'bg-white border-gray-200 text-gray-900'
                  } hover:border-gray-400`}
            ...
            >
              Showing {filteredAndSortedTrades.length} trade
              {filteredAndSortedTrades.length !== 1 ? 's' : ''}
              {hasActiveFilters &&
                ` (filtered from ${loggedTrades.length} total)`}
            </div>
          </div>
                ...
                <table className="w-full min-w-[1000px]">
                  <thead>
                    ...
                    >
                      ...
                      <th
                        className={`text-left py-3 px-4 text-xs uppercase tracking-wider font-medium cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 ${
                      ...
                      <th
                        className={`text-right py-3 px-4 text-xs uppercase tracking-wider font-medium cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 ${
                          isDarkMode ? 'text-gray-400' : 'text-gray-500'
                        }`}
                      ...
                      <th
                        className={`text-right py-3 px-4 text-xs uppercase tracking-wider font-medium ${
                          isDarkMode ? 'text-gray-400' : 'text-gray-500'
                        }`}
                      >
                        Stop
                  ...
                  <tbody
                    className={`${isDarkMode ? 'divide-gray-700' : 'divide-gray-100'} divide-y`}
                  >
                    {paginatedTrades.map((trade) => (
                      <tr
                        key={trade.id}
                        className={`group ${isDarkMode ? 'hover:bg-gray-700/50' : 'hover:bg-gray-50'}`}
                      >
                        <td className="py-3 px-4 whitespace-nowrap">
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              onClick={() =>
                                trade.id && onDeleteTrade(trade.id)
                              }
                        ...
                        <td
                          className={`py-3 px-4 text-sm whitespace-nowrap ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}
                        >
                          {trade.date}
                        </td>
                        ...
                        <td
                          className={`py-3 px-4 text-sm font-medium text-right ${isDarkMode ? 'text-gray-200' : 'text-gray-900'}`}
                        >
                          {currencySymbol}
                        ...
                        <td
                          className={`py-3 px-4 text-sm text-right ${isDarkMode ? 'text-gray-200' : 'text-gray-900'}`}
                        >
                        ...
                        <td
                          className={`py-3 px-4 text-sm font-semibold text-right ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`}
                        >
                          {trade.rrRatio !== null ? (
